...
func (engine *Engine) GET(aPath string, aHandler HandlerFunc) {
	engine.RouterMgr.addRouter("GET", aPath, aHandler)
}

// POST 添加Post请求路由
func (engine *Engine) POST(aPath string, aHandler HandlerFunc) {
	engine.RouterMgr.addRouter("POST", aPath, aHandler)
}
...
func (engine *Engine) ServeHTTP(aRspW http.ResponseWriter, aReq *http.Request) {
	...
	// 先将匹配的中间件方法加入到切片中
	for _, mGroup := range engine.AllGroups {
		// 检查 参数1 是否以 参数2 开头的
	...
	// 然后再执行路由方法
	engine.RouterMgr.ExecHandleFunc(mContext)
}
